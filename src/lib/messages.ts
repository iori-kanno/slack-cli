...
\`\`\`
Command:
  slack-cli aggregate:members-reacted 指定された期間内に指定されたリアクションを最も行ったユーザー上位 5位までリストアップする
  slack-cli aggregate:reactions       指定された期間内に指定されたリアクション数が多いユーザーを上位 5位までリストアップする
  slack-cli archive:channel           指定されたパブリックチャンネルをアーカイブする
  slack-cli archive:inactive-channels 指定された期間以上更新のないパブリックチャンネル全てをアーカイブする
  ...
  slack-cli pulse:hearing             スプレッドシートから対象の UserId を取得して DM で最近の調子を訪ね、回答をスプレッドシートに書き込む
